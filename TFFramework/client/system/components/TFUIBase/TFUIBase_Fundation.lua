...
function TFUIBase:getChild(path)
	-- if not self.children__[path] then
		-- assert(false, string.format("%s is not exist" , path))
	-- end
	return self.children__ and self.children__[path] or self:getChildByName(path)
end
...
function TFUIBase:luaclone(bIsContainChild)
