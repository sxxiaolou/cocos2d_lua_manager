...
local Spawn 				= Spawn
local CallFuncN 			= CallFuncN
local DelayTime 			= DelayTime
local Repeat 				= Repeat
...
local tActionFunctions = {
	...
	["Show"]			= tGetActionFunction.getShowAction,
	["Hide"]  			= tGetActionFunction.getHideAction
}
...
local function generateAction(target, tween)
	tween.duration = tween.duration or 0
	...
	for k, v in pairs(tween) do
		if type(v) == 'table' and k ~= 'bezier' then
			v.duration = v.duration or 0
			local innerTarget = v.target or target
			act = generateAction(innerTarget, v)
			if innerTarget ~= target then 
				act = TargetedAction:create(innerTarget, act)
			end
			table.insert(carr, act)
		end
	end

	if #carr > 0 then
		carr = Spawn:createWithTable(carr)
	end
	...
	if tween.repeated then
		...
		carr = Repeat:create(carr, nRepeatCount)
	end

	-- Delay
	if tween.delay then
		act = DelayTime:create(tween.delay)
		table.insert(seqArr, act)
	end
	
	if type(carr) ~= 'table' then
		if tween.reverse then
			carr = carr:reverse()
		end
		table.insert(seqArr, carr)
	end

	-- onComplete
	if tween.onComplete and type(tween.onComplete) == 'function' then
		act = CallFuncN:create(function () 
	...
	local seq = Sequence:createWithTable(seqArr)
	return seq
end
...
function TFTweenManager:createAction(tween, bIsByAction)
    ...
    if tolua.isnull(tween.target) then return nil end
	...
	if tween.follow then -- this action can only be used alone
		...
		local act = TFFollow:create(follow.followedWidget, ccr(follow.rect[1], follow.rect[2], follow.rect[3], follow.rect[4]))
	...
	for k, v in pairs(tween) do
		if type(v) == 'table' then
			...
			if innerTarget ~= nil then
				local objAction = generateAction(innerTarget, v)
				if innerTarget ~= tween.target then 
					objAction = TargetedAction:create(innerTarget, objAction)
				end
	...
	for nIdx = 1, #tActions do
		table.insert(seqArr, tActions[nIdx])
	end
	seq = Sequence:createWithTable(seqArr)
	...
	if tween.repeated then
		...
		seq = Repeat:create(seq, nRepeatCount)
	end

	--onComplete
	if tween.onComplete and type(tween.onComplete) == 'function' then
		act = CallFuncN:create(tween.onComplete)
		seqArr = {}
		table.insert(seqArr, seq)
		table.insert(seqArr, act)
		seq = Sequence:createWithTable(seqArr)
	end

	if tween.speed then -- this action can only be used at top
		seq = Speed:create(seq, tween.speed)
		return seq
	end
...
local function reverseTween(tween, target)
	...
	for k, v in pairs(tween) do
		if type(v) == 'table' then
			...
			local x, y = target:getPositionX(), target:getPositionY()
			if v.x then 
				ta = x
				target:setPosition(ccp(v.x, y)) 
				v.x = ta
			else
				v.x = x
			end
