...
function EditLua:createTextField(szId, tParams)
	...
	textField._LabelFontFillColor = ccc3(255, 255, 255)
	textField._LabelMixColor		= ccc3(255, 255, 255)
	
	textField._useOutline = false
	textField._useShadow = false
	textField._outLineSize = 1
	textField._outLineColor = ccc3(0, 0, 0)
	textField._shadowSize = ccs(0, 0)
	textField._shadowColor = ccc3(0, 0, 0)

	print(tolua.type(textField))
...
function tMETextField:useOutline(szId, tParams)
	print("setUseOutline")
	targets[szId]._useOutline = tParams.bRet
	if targets[szId]._useOutline then
		targets[szId]:enableStroke(targets[szId]._outLineColor, targets[szId]._outLineSize)
	else
		targets[szId]:disableStroke()
	end
	print("setUseOutline success")
end	

function tMETextField:setOutlineColor(szId, tParams)
	print("setOutlineColor")
	targets[szId]._outLineColor = ccc3(tParams.nR, tParams.nG, tParams.nB)
	if targets[szId]._useOutline then
		targets[szId]:enableStroke(targets[szId]._outLineColor, targets[szId]._outLineSize)
	end
	print("setOutlineColor success")
end	

function tMETextField:setOutlineSize(szId, tParams)
	print("setOutlineSize")
	targets[szId]._outLineSize = tParams.nSize
	if targets[szId]._useOutline then
		targets[szId]:enableStroke(targets[szId]._outLineColor, targets[szId]._outLineSize)
	end
	print("setOutlineSize success")
end	

function tMETextField:useShadow(szId, tParams)
	print("setUseShadow")
	targets[szId]._useShadow = tParams.bRet
	if targets[szId]._useShadow then
		targets[szId]:enableShadow(targets[szId]._shadowColor, targets[szId]._shadowSize)
	else
		targets[szId]:disableShadow()
	end
	print("setUseShadow success")
end	

function tMETextField:setShadowColor(szId, tParams)
	print("setShadowColor")
	targets[szId]._shadowColor = ccc3(tParams.nR, tParams.nG, tParams.nB)
	if targets[szId]._useShadow then
		targets[szId]:enableShadow(targets[szId]._shadowColor, targets[szId]._shadowSize)
	end
...
function tMETextField:setShadowSize(szId, tParams)
	print("setShadowSize")
	targets[szId]._shadowSize = ccs(tParams.nWidth, tParams.nHeight)
	if targets[szId]._useShadow then
		targets[szId]:enableShadow(targets[szId]._shadowColor, targets[szId]._shadowSize)
	end
	print("setShadowSize success")
end	
