	...
	function TFDirector.hook(tp)
        if tp == 'call' then
		    ...
		    if tProfile.tNames[func] == nil then
                ...
                tProfile.tTimers[szName] = {0, TFLuaTime:clock()}
		    else
                ...
                if not kickFilter[name] then 
                    tProfile.tTimers[szName][2] = TFLuaTime:clock()
                end
		    end
        elseif tp == "return" then
			...
			tProfile.tCounters[szName] = tProfile.tCounters[szName] + 1
            if not kickFilter[name] then 
                tProfile.tTimers[szName][1] = (tProfile.tTimers[szName][1] + TFLuaTime:clock() - tProfile.tTimers[szName][2])-- / 2
...
function TFDirector:createDebugerLayer(objScene)
	if self.testDebuger then
		...
		local function updateCounters()
			...
			testDebuger.luaMemLabel 		:setString(string.format('Lua Mem: %.2fM', collectgarbage("count")/1024))	
		end
