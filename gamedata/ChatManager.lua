...
local ChatManager = class("ChatManager", BaseManager)
...
ChatManager.kSpeakerGoodsIdKey = "Chat.Trumpet"
...
function ChatManager:sendPublic(chat )	
    ...
    local openLevle = FunctionOpenConfigure:getLevel("Chat.Public.Level") --ConstantData:getValue("Chat.Public.Level")
    local openVipLevle = ConstantData:getValue("Chat.Public.VipLevel")
...
function ChatManager:sendServerPublic(chat )
	...
	if ServerSwitchManager:getServerSwitchStatue(ServerSwitchType.ServerChat ) ==false then
...
function ChatManager:fillMyMsg(chatType, content)
	...
	local roleID = MainPlayer:getSex()
	local roleInfo = CardData:objectByID(roleID)
 ...
 --   	end
	...
	self.myMsg.name      = MainPlayer:getName()
	...
	self.myMsg.icon 	 = MainPlayer:getUseIcon()
	self.myMsg.headPicFrame = MainPlayer:getFrameId()
...
function ChatManager:getSpeaker()
	local speakerConstant = ConstantData:objectByID(ChatManager.kSpeakerGoodsIdKey)
	return BagManager:getGoodsByTplId(speakerConstant.res_id)
end
...
function ChatManager:showPrivateChatLayer(id)
	self:setPrivateFriendId(id)

    local layer = AlertManager:addLayerByFile("lua.logic.chat.ChatMainLayer",AlertManager.BLOCK_AND_GRAY,AlertManager.TWEEN_1)
    layer:changeGroupChoice(4)
    self:requestPrivateChatList()
    layer.panelFriendList:setVisible(true)
    AlertManager:show()
    return layer
end

-- 
function ChatManager:setPrivateFriendId( id )
	self.PrivateFriendId = id 
end

function ChatManager:getPrivateFriendId()
	return self.PrivateFriendId
end
...
function ChatManager:getNewMessageList()
	return self.newMessageListnewMessageList
end
