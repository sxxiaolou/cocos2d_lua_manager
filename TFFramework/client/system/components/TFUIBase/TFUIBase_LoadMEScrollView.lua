...
function TFUIBase:initMEScrollView_MEEDITOR(pval, parent)
	...
	if val['scrollbarTexturePath'] ~= nil and self.setScrollBarTexture then
		self:setScrollBarTexture(val['scrollbarTexturePath'])
	end

	if val['showScrollbar'] ~= nil and self.setShowScrollBarEnabled then
		self:setShowScrollBarEnabled(val['showScrollbar'] == "True" or val['showScrollbar'] == true)
	end
