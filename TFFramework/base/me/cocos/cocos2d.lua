...
function me.pointMT:getX()
    return self.x
end

function me.pointMT:getY()
    return self.y
end
...
function me.pAdd(pt1,pt2)
    if not pt2 then return pt1 end
...
me.pointMT.distance                 = me.pGetDistance
...
function me.rectEquals(rect1, rect2)
    return abs(rect1.origin.x - rect2.origin.x) < FLT_EPSILON 
        and abs(rect1.origin.y - rect2.origin.y) < FLT_EPSILON 
        and abs(rect1.size.width - rect2.size.width) < FLT_EPSILON 
        and abs(rect1.size.height - rect2.size.height) < FLT_EPSILON 
end
