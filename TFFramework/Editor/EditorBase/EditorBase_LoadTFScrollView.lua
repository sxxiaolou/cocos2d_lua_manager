...
setmetatable(tMEScrollView, require("TFFramework.Editor.EditorBase.EditorBase_LoadTFPanel"))
...
function tMEScrollView:setScrollBarTexture(szId, tParams)
	...
	if tParams.szSbTexture ~= nil and targets[szId] ~= nil and targets[szId].setScrollBarTexture then
		targets[szId]:setScrollBarTexture(tParams.szSbTexture)
		print("setScrollBarTexture run success")
	end
end

function tMEScrollView:setShowScrollBarEnabled(szId, tParams)
	print("setShowScrollBarEnabled")
	if tParams.bRet ~= nil and targets[szId] ~= nil and targets[szId].setShowScrollBarEnabled then
		targets[szId]:setShowScrollBarEnabled(tParams.bRet)
		targets[szId]:setScrollBarAutoHide(false)
		print("setShowScrollBarEnabled run success")
	end
end
