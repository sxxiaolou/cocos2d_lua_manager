...
function TFUIBase:initAction(ui, val)
			...
			for _, uc in pairs(v) do 
				if type(uc) == 'table' then 
					...
					if comp and uc.frames then 
						--by liu.tong
						comp.originScaleX = comp:getScaleX()
						comp.originScaleY = comp:getScaleY()
						comp.animationFrames__ = comp.animationFrames__ or {}
						comp.animationFrames__[name] = comp.animationFrames__[name] or {}
						-- tony 2017.1.11
						comp.__nDT = comp.__nDT or 0
						comp.__enterTime = comp.__enterTime or 0
						...
						for _, frame in pairs(uc.frames) do 
							...
							comp.__enterTime = frame.enterTime > comp.__enterTime and frame.enterTime or comp.__enterTime
						end
	...
	function ui:updateToFrame(szAction, nFrame)
		...
		self:setActionBaseAttribute(model, szAction)
	...
	function ui:runAnimation(name, nRound)
		...
		self:setActionBaseAttribute(model, name)
	...
	--by jin.zhiqing
	function ui:setActionBaseAttribute(model, name)
		if not model or not model.actions[name] then return end

		local actionModel = model.actions[name].actionModel
		if actionModel then 
			for _, comp in pairs(actionModel) do
				comp._actionBaseAttribute = comp._actionBaseAttribute or {}
				comp._actionBaseAttribute.scaleX = comp.originScaleX or comp._actionBaseAttribute.scaleX or comp:getScaleX()
				comp._actionBaseAttribute.scaleY = comp.originScaleY or comp._actionBaseAttribute.scaleX or comp:getScaleY()
			end
		end
	end
