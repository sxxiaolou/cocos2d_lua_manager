...
function TFUIBase:initMETextButton_MEEDITOR(pval, parent)
	...
	if val['titletext'] ~= " " or val['titletext'] ~= "" then 
		...
		if val['useOutline'] == "True" then
			local color = val['outlineColor']
			local r = ('0x' .. color['4:5']) + 0
			local g = ('0x' .. color['6:7']) + 0
			local b = ('0x' .. color['8:9']) + 0
			local size = tonumber(val['outlineSize'])
			self:setFontStrokeEnabled(ccc3(r, g, b), size)
		end

		if val['useShadow'] == "True" then
			local color = val['shadowColor']
			local r = ('0x' .. color['4:5']) + 0
			local g = ('0x' .. color['6:7']) + 0
			local b = ('0x' .. color['8:9']) + 0
			local size =ccs(tonumber(val['shadowWidth']), tonumber(val['shadowHeight']))
			self:setFontShadowEnabled(ccc3(r, g, b), size)
		end

		self:initMEColorProps(pval, parent)

		--by wcx. 2016.6.12 :修改内容 添加button上文字的描边效果
		if val["DiyPropertyViewModel"] and val["DiyPropertyViewModel"]["outlineSize"] and val["DiyPropertyViewModel"]["outlineSize"] > 0 then
			local color = val["DiyPropertyViewModel"]['outlineColor']
			local r = ('0x' .. color['4:5']) + 0
			local g = ('0x' .. color['6:7']) + 0
			local b = ('0x' .. color['8:9']) + 0
			self:setFontStrokeEnabled(ccc3(r, g, b), val["DiyPropertyViewModel"]["outlineSize"])
		elseif val["DiyPropertyViewModel"] and val["DiyPropertyViewModel"]["outlineSize"] == 0 then
		--注释默认描边 modify by wcx 2016/12/12--
		-- else
		-- 	local text = self:getText()
		-- 	if text ~= "" then
		-- 		self:setFontStrokeEnabled(GameConfig.outline_blue.color,GameConfig.outline.size);
		-- 	end
		end
	end
end
