...
function TFUIBase:initMELabel_MEEDITOR(pval, parent)
	...
	if val['nTextAlign'] then
		self:setTextVerticalAlignment(val['nTextAlign'] + 0)
	end
	if val['nTextHAlign'] then
		self:setTextHorizontalAlignment(val['nTextHAlign'] + 0)
	end
	...
	if val['fontName'] and self.setFontName then 
		if "font/game_font2.TTF" == val['fontName'] then 
	 		val['fontName'] = "font/ttf/game_font.ttf"
	 	end
	...
	if not bIsIgnore then
		if (val['width'] or val['height']) then 
			local width = val['width'] + 0 or 0
			local height = val['height'] + 0 or 0
			self:setTextAreaSize(ccs(width, height))
		end
	end	
	...
	--by daxiong. 2016.6.6 : 修改内容 添加描边效果
	if val['fontStroke'] and type(val['fontStroke']) == 'table' and val['fontStroke']['IsStroke'] then
		...
		self:setOutline(ccc3(r, g, b), val['fontStroke']['StrokeSize']);
	end
end
