...
--[[--
	获取数组元素的逆向迭代, 用在 for 语句
	@return 迭代函数
]]	
function TFArray:riterator()
	local s, e = self.head, self.tail
	return function ()
		e = e - 1
		if s > e then return nil end
		return self.m_list[e]
	end
end
...
function TFArray:setAt(nIndex, val)
	self.m_list[self.head + nIndex - 1] = val
end
...
TFArray.removeAt = TFArray.removeObjectAt
...
function TFArray:shell( cmp )
	if cmp == nil then
		return
	end
	local h = 1;
	local len = self:length()
	while  h < math.floor(len/3) do
		h = h*3+1;
	end
	while h >= 1 do
		for i = h,len-1 do
			for j = i, h , -h do
				if cmp(self:getObjectAt(j+1) ,self:getObjectAt(j-h+1)) then
					self:swap(j+1, j-h+1)
				end
			end
		end
