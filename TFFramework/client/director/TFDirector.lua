...
-- for xuanyuanjian
function TFDirector:SaveShareTexture(objUI)
	if not objUI then return nil end

	local filename = CCFileUtils:sharedFileUtils():getWritablePath().."shareTex.jpg"
	print("SaveShareTexture:"..filename)
	local size = objUI:getSize()
	local objRendTexture = CCRenderTexture:create(size.width, size.height, kCCTexture2DPixelFormat_RGBA8888)
	local oldPos = objUI:getPosition()
	oldPos = ccp(oldPos.x, oldPos.y)
    ...
    objRendTexture:endToLua()
    objUI:setPosition(oldPos)
    objRendTexture:saveToFile(filename)
    return filename
end
...
local __retainObjs = {}
function TFDirector:addRetainObj(obj, lock)
	if not __retainObjs[obj] then 
	    if not lock then obj:retain() end
	    __retainObjs[obj] = true
	end
end

function TFDirector:removeRetainObj(obj, lock)
    if __retainObjs[obj] then 
        if not lock then obj:release() end
        __retainObjs[obj] = nil
    end
end

function TFDirector:clearRetainObjs()
    for obj in pairs(__retainObjs) do
...
function TFDirector:disableDiviceSleep(bNotSleep)
	...
	elseif CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID then
		TFLuaOcJava.callStaticMethod("org/cocos2dx/TerransForce/TerransForce", "disableDeviceSleep", {bNotSleep})
	end
end
