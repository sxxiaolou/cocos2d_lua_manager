...
if CC_TARGET_PLATFORM == CC_PLATFORM_IOS then
    ...
    HeitaoSdk.classname = iOSSDKClassName:get()
...
--用于在iOS工程中去掉heitao关键词
function iOSSDKCallBack(result, msg)
    HeitaoSDKCallBack(result, msg)
end
...
function HeitaoSdk.isLogined(doLoginWhenNotLogin)
    local args = nil
    if CC_TARGET_PLATFORM == CC_PLATFORM_IOS then
        args = {doLoginWhenNotLogin = doLoginWhenNotLogin}
    else
        args = {doLoginWhenNotLogin}
    end
    local ok,ret = TFLuaOcJava.callStaticMethod(HeitaoSdk.classname, "isLogined", args, "(Z)Z")
...
function HeitaoSdk.roleNameChanged(roleName)
    ...
    local ok,ret = TFLuaOcJava.callStaticMethod(HeitaoSdk.classname, "roleNameChanged", args, "(Ljava/lang/String;)V")
...
--     NSString *cpExtendInfo= [param objectForKey:@"cpExtendInfo"];   //商品描述  //CP扩展信息
        ...
        args = {
            ...
            unitName    = itemName,
            productId   = string.format("%d", productId),
...
function  HeitaoSdk.shareWxText(type_id, strTitle, strText, strDesc, linkURL, imagePath)
   --[[* 注：
