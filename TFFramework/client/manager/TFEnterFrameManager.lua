...
function TFEnterFrameManager:addEnterFrameEvent(enterFrameFunc, ...)
	...
	if TFEnterFrameManagerModel.tHandles:indexOf(enterFrameFunc) == -1 then
		...
		--print("TFEnterFrameManager => Add a new enterFrameEvent.")
	end
	if TFEnterFrameManagerModel.removeFuncBuff and 
		TFEnterFrameManagerModel.removeFuncBuff:indexOf(enterFrameFunc) ~= -1 then
		TFEnterFrameManagerModel.removeFuncBuff:removeObject(enterFrameFunc)
	end
    return enterFrameFunc
end
