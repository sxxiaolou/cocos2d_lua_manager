...
ErrorCodeManager.ErrorCode 	= "ErrorCodeManager.ErrorCode"
...
-- TextIndex.BAN                                           					   = 3359     --您已被禁止登录
	-- 服务器已满  主动断开连接
	print("ErrorCodeManager" , data)
	-- print(TextIndex.server_refuse_service)
	...
	TFDirector:dispatchGlobalEventWith(ErrorCodeManager.ErrorCode, data)
end
...
function ErrorCodeManager:reportErrorMsg(errorMsg)
    hjReportLuaException(tostring(errorMsg), debug.traceback())
