...
function EditMapData:onTouchMap(pos)
	if TFDirector.bCTRLDown then 	--移动地图
		--print("move map")
		local curPos = targets["root"]:getPosition()
		local convertPos = ccpSub(pos, self.startPos)
		...
		if target.nGridMode == 0 then
			EditMapData.setSelfGridColor(nil, pos, target.nGridValue)
		elseif target.nGridMode == 1 then
			EditMapData.removeGridColor(nil, pos, target.nGridValue)
		elseif target.nGridMode == 2 then
			EditMapData.removeGridAllColor(nil, pos)
		elseif target.nGridMode == 3 then
			EditMapData.removeAllGridColor(nil, pos)
		end
	end
end
...
function EditMapData:onTouchBegan( ... )
	...
	EditMapData.onTouchMap(self, self.startPos)
end

function EditMapData:onTouchMoved( ... )
	--print("touch move")
	local movePos = self:getTouchMovePos()
	EditMapData.onTouchMap(self, movePos)
...
function EditMapData:addGrid( ... )
	-- setGridLine
	local point = {}
	point.x = target:getAnchorPoint().x * target.originSize.width * -1
	point.y = target:getAnchorPoint().y * target.originSize.height * -1
	...
	for j = 0, target.originSize.width / target.nGridWidth do	
	-- 	image = TFImage:create("test/imageGrid.png")
	-- 	image:setScale9Enabled(true)
	-- 	image:setAnchorPoint(ccp(0, 0))
	-- 	image:setColor(ccc3(255, 0, 0))
	-- 	image:setSize(CCSize(2, target.originSize.height))
	...
	image:setOpacity(30)
		...
		image:setSize(CCSizeMake(target.originSize.width, target.originSize.height))
	-- image:setCoordsSize(target.originSize.width, target.originSize.height, 0, 0)
	image:setAnchorPoint(ccp(0, 0))
...
function EditMapData:saveData(szId, tParams)
	...
	local function doSave()
		...
		if tParams.szSavePath and target then
			local objMap = Astar_Hash(target.nCol, target.nRow)
			print(target.nCol, target.nRow)
			--方式2
			local nX, nY
			for i in tDrawIndexs:iterator() do
				nX = i % target.nCol
				nY = EditorUtils:GetIntPart(i/target.nCol)
				if target.tColorTotle[i] and target.tColorTotle[i] ~= 0 then
					objMap:set(nY, nX, target.tColorTotle[i])
				end
			end
			objMap:exportFile(tParams.szSavePath, target.originSize.width, target.originSize.height)
			print("saveData success")
		end
		bIsSaving = false
	end

	xpcall(doSave, __G__TRACKBACK__)
end
...
function EditMapData:loadData(szId, tParams)
	...
	if tParams.szSavePath ~= "" and szCurMap ~= tParams.szSceneID then
		local objMap = AStar()
		local bIsOpen = objMap:init(tParams.szSavePath)
		...
		if bIsOpen then
			...
			tMapData.nWidth, tMapData.nHeight = objMap:getMapWidth(), objMap:getMapHeight()

			local width, height = objMap:getWidth(), objMap:getHeight()
			print("openFile success", width, height)
			if width == 0 then width = 20 end
			if height == 0 then height = 20 end
			local nGridWidth = EditorUtils:GetIntPart(tMapData.nWidth / width)
			local nGridHeight = EditorUtils:GetIntPart(tMapData.nHeight / height)

			target.nGridWidth = nGridWidth
			target.nGridHeight = nGridHeight
			tMapData.nGridWidth, tMapData.nGridHeight = nGridWidth, nGridHeight
			...
			if target.nGridWidth ~= 0 then
				...
				for i = 0, height - 1 do 
					for j = 0, width - 1 do 
						local nTag = i * width + j				
						--if tDrawIndexs:indexOf(nTag) == -1 then
							tDrawIndexs:push(nTag)
						--end
	
						local isBlock = objMap:get(i, j)
						target.tColorTotle[nTag] = isBlock

						if isBlock ~= 0 then 
							local nX = j * target.nGridWidth
							local nY = i * target.nGridHeight
...
function EditMapData:setGridEnabled(szId, tParams)
	...
	if tParams.bRet ~= nil and tParams.nWidth ~= nil and tParams.nHeight ~= nil then
		...
		if tParams.bRet then
			...
			target:setPosition(targets["root"]:getPosition())
			target.originSize = targets["root"]:getSize()
			target:setSize(target.originSize)--(CCSize(5000, 5000))
			...
			target.nRow = EditorUtils:GetIntPart(target.originSize.height / target.nGridHeight)
			target.nCol = EditorUtils:GetIntPart(target.originSize.width / target.nGridWidth)
